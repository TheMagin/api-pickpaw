...
export default class SocialMediasController {
  public async store({ request, response }: HttpContextContract) {
    const { socialMedia: data } = await request.validate({
      schema: CreateSchema,
    });
  ...
  public async index({ request, response, bouncer }: HttpContextContract) {
    ...
    try {
      const socialMedia = await SocialMedia.filter(filters).paginate(
        page,
        limit
      );
  ...
  public async update({ request, params, response }: HttpContextContract) {
    ...
    socialMediaModel.type_social_media_id =
      data.typeSocialMediaId ?? socialMediaModel.type_social_media_id;
